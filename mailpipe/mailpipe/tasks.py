...
def notify_callback(email_id, retry_seconds=None, default_retry_delay=60):
    try:
        retry_seconds = retry_seconds or getattr(settings, 'MAILPIPE_CALLBACK_RETRY_SECONDS', 60)
        from models import Email
        ...
        if retry and retry_seconds < getattr(settings, 'MAILPIPE_CALLBACK_TIMEOUT_SECONDS', 60 * 60):
            notify_callback.apply_async([email_id], {'retry_seconds': retry_seconds}, countdown=retry_seconds)
        return not retry
